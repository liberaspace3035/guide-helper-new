-- 要件定義書に基づく追加フィールドのマイグレーション（修正版・PostgreSQL版）
-- このファイルは add_requirements_fields.sql と同じ内容ですが、PostgreSQL用に最適化されています
...
BEGIN
    IF NOT EXISTS (
        ...
        ALTER TABLE users ADD COLUMN postal_code VARCHAR(10) NULL;
...
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM information_schema.columns 
        WHERE table_name = 'user_profiles' AND column_name = 'interview_date_1'
    ) THEN
        ALTER TABLE user_profiles ADD COLUMN interview_date_1 TIMESTAMP NULL;
...
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM information_schema.columns 
        WHERE table_name = 'user_profiles' AND column_name = 'interview_date_2'
    ) THEN
        ALTER TABLE user_profiles ADD COLUMN interview_date_2 TIMESTAMP NULL;
...
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM information_schema.columns 
        WHERE table_name = 'user_profiles' AND column_name = 'interview_date_3'
    ) THEN
        ALTER TABLE user_profiles ADD COLUMN interview_date_3 TIMESTAMP NULL;
...
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM information_schema.columns 
        WHERE table_name = 'user_profiles' AND column_name = 'application_reason'
    ) THEN
        ALTER TABLE user_profiles ADD COLUMN application_reason TEXT NULL;
...
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM information_schema.columns 
        WHERE table_name = 'user_profiles' AND column_name = 'visual_disability_status'
    ) THEN
        ALTER TABLE user_profiles ADD COLUMN visual_disability_status TEXT NULL;
...
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM information_schema.columns 
        WHERE table_name = 'user_profiles' AND column_name = 'disability_support_level'
    ) THEN
        ALTER TABLE user_profiles ADD COLUMN disability_support_level VARCHAR(10) NULL;
...
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM information_schema.columns 
        WHERE table_name = 'user_profiles' AND column_name = 'daily_life_situation'
    ) THEN
        ALTER TABLE user_profiles ADD COLUMN daily_life_situation TEXT NULL;
...
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM information_schema.columns 
        WHERE table_name = 'guide_profiles' AND column_name = 'application_reason'
    ) THEN
        ALTER TABLE guide_profiles ADD COLUMN application_reason TEXT NULL;
...
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM information_schema.columns 
        WHERE table_name = 'guide_profiles' AND column_name = 'goal'
    ) THEN
        ALTER TABLE guide_profiles ADD COLUMN goal TEXT NULL;
...
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM information_schema.columns 
        WHERE table_name = 'guide_profiles' AND column_name = 'qualifications'
    ) THEN
        ALTER TABLE guide_profiles ADD COLUMN qualifications TEXT NULL;
...
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM information_schema.columns 
        WHERE table_name = 'guide_profiles' AND column_name = 'preferred_work_hours'
    ) THEN
        ALTER TABLE guide_profiles ADD COLUMN preferred_work_hours TEXT NULL;
...
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM information_schema.columns 
        WHERE table_name = 'requests' AND column_name = 'nominated_guide_id'
    ) THEN
        ALTER TABLE requests ADD COLUMN nominated_guide_id INTEGER NULL;
...
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM information_schema.columns 
        WHERE table_name = 'matchings' AND column_name = 'report_completed_at'
    ) THEN
        ALTER TABLE matchings ADD COLUMN report_completed_at TIMESTAMP NULL;
